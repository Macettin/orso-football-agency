...
export async function PlayerCard({player, locale}: {player: Player; locale: Locale}) {
  ...
  return (
    <article className="light-card group mx-auto w-full max-w-[330px] overflow-hidden border transition duration-300 hover:-translate-y-1 hover:border-accent/60 hover:shadow-blue">
      {player.photoUrl ? (
        <div className="relative aspect-[4/5] overflow-hidden border-b border-[#d7e1ee] bg-[radial-gradient(circle_at_50%_35%,rgba(37,99,235,.18),transparent_48%),linear-gradient(180deg,#dce8f7_0%,#f5f8fc_100%)] p-4 sm:p-5">
          <Image
            src={player.photoUrl}
            alt={player.name}
            fill
        ...
        <div className="aspect-[4/5] border-b border-[#d7e1ee]">
      ...
      <div className="p-4 sm:p-5">
        ...
        <h3 className="mt-1.5 font-display text-xl font-semibold leading-tight text-[#0F172A]">{player.name}</h3>
        ...
        <Link
          href={`/players/${slug}`}
          className="mt-4 flex items-center justify-between border-t border-[#d7e1ee] pt-3.5 text-[10px] font-bold uppercase tracking-[0.14em] text-[#003090] transition group-hover:text-blue-600"
        >
          {t('viewProfile')}
